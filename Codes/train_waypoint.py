...
class TrainConfig:
    ...
    log_interval: int = 10
    save_path: str = "ppo_quad_waypoint.pt"
    init_path: str = "ppo_quad_hover.pt"   # warm-start from hover
    seed: int = 0


def make_env(seed: int = 0) -> QuadWaypointEnv:
    env = QuadWaypointEnv(xml_path="drone.xml", seed=seed, max_steps=200)
    return env
...
def main():
    ...
    # Warm-start from hover policy
    try:
        state_dict = torch.load(cfg.init_path, map_location=agent.device)
        agent.ac.load_state_dict(state_dict)
        print(f"Loading initial weights from {cfg.init_path}")
    except FileNotFoundError:
        print(f"WARNING: {cfg.init_path} not found. Training waypoint from scratch.")
    ...
    print(f"Saved waypoint policy to {cfg.save_path}")
